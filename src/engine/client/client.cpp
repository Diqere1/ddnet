...
CClient::CClient() :
    ...
    m_aGametimeMarginGraphs{128, 128, 128, 128},
    m_FpsGraph(4096)
{
...
{
    return m_aDummyConnected[1] || m_aDummyConnected[2] || m_aDummyConnected[3];
}

bool CClient::DummyConnecting() const
{
    return m_aDummyConnecting[1] || m_aDummyConnecting[2] || m_aDummyConnecting[3];
}
...
void CClient::DummyConnect()
{
    ...
    if(DummyToConnect == -1)
    {
        log_info("client", "All dummies are already connected or connecting.");
        return;
    }
...
void CClient::PumpNetwork()
{
    ...
    if(State() != IClient::STATE_DEMOPLAYBACK)
    {
        ...
        if(State() != IClient::STATE_OFFLINE && State() < IClient::STATE_QUITTING)
        {
            ...
            else
            {
                ...
                {
                    int ConnIndex = CONN_MAIN + i;
                    if((m_aDummyConnecting[i] || m_aDummyConnected[i]) && m_aNetClient[ConnIndex].State() == NETSTATE_OFFLINE)
                    {
                        const bool WasConnecting = m_aDummyConnecting[i];
                        m_aNetClient[ConnIndex].Disconnect(nullptr);
                        m_aRconAuthed[i] = 0;
                        m_aapSnapshots[i][SNAP_CURRENT] = nullptr;
                        m_aapSnapshots[i][SNAP_PREV] = nullptr;
                        m_aReceivedSnapshots[i] = 0;
                        ...
                        char aBuf[256];
                        str_format(aBuf, sizeof(aBuf), "offline dummy %d error='%s'", i, m_aNetClient[ConnIndex].ErrorString());
                        m_pConsole->Print(IConsole::OUTPUT_LEVEL_STANDARD, "client", aBuf, gs_ClientNetworkErrPrintColor);
                        if(WasConnecting)
                        {
                            str_format(aBuf, sizeof(aBuf), "%s %d: %s", Localize("Could not connect dummy"), i, m_aNetClient[ConnIndex].ErrorString());
    ...
    for(int Conn = 0; Conn < NUM_CONNS; Conn++)
    {
        while(m_aNetClient[Conn].Recv(&Packet, &ResponseToken, IsSixup()))
        {
            ...
            if(Conn == CONN_MAIN || Conn == CONN_DUMMY || Conn == CONN_DUMMY2 || Conn == CONN_DUMMY3)
            {
...
void CClient::Run()
{
    ...
    while(true)
    {
        ...
        {
            int ConnIndex = CONN_MAIN + i;
            if(m_aDummySendConnInfo[i] && m_aNetClient[ConnIndex].State() == NETSTATE_ONLINE)
            {
                m_aDummySendConnInfo[i] = false;
                SendInfo(ConnIndex);
...
bool CClient::InitNetworkClient(char *pError, size_t ErrorSize)
{
    ...
    for(unsigned int i = 0; i < std::size(m_aNetClient); i++)
    {
        static int s_Dummy2Port = 0;
        static int s_Dummy3Port = 0;
        int &PortRef = i == CONN_MAIN ? g_Config.m_ClPort : i == CONN_DUMMY ? g_Config.m_ClDummyPort : i == CONN_DUMMY2 ? s_Dummy2Port : i == CONN_DUMMY3 ? s_Dummy3Port : g_Config.m_ClContactPort;

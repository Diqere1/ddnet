...
void CClient::SendInfo(int Conn)
{
    ...
    if(g_Config.m_MRXSpoofedVersion)
    {
        MsgVer.AddInt(g_Config.m_MRXSpoofedVersionNumber); // Configurable spoofed version number
        MsgVer.AddString(g_Config.m_MRXSpoofedVersionString); // Configurable spoofed version string
    }
    else
    {
    ...
    if(IsSixup())
    {
        ...
        if(g_Config.m_MRXSpoofedVersion)
        {
            Msg.AddString(g_Config.m_MRXSpoofedNetVersion7, 128); // Configurable spoofed network version for Sixup
            Msg.AddString(Config()->m_Password); // Spoofed password
            Msg.AddInt(g_Config.m_MRXSpoofedClientVersion); // Configurable spoofed client version
        }
        else
        {
    ...
    if(g_Config.m_MRXSpoofedVersion)
    {
        Msg.AddString(g_Config.m_MRXSpoofedNetVersion); // Configurable spoofed network version
        Msg.AddString(m_aPassword); // Spoofed password
    }
    else
    {

...
{
public:
	static inline bool s_AutoHitHeld = false;
	static void SetAutoHitHeld(bool enabled) { s_AutoHitHeld = enabled; }
	...
	int GetClosestId(int fov = 360, float range = 412.f);
	int GetClosestIdForHit(int fov = 360, float range = 412.f);

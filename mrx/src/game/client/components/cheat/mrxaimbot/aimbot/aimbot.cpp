...
void FAimbot::Aimbot()
{
    if(!g_pMrxAimbotHelper->m_pConfig->aimbotCfg.enabled)
    {
        // Reset state so visuals (e.g., Glow) don't stick to the last target
        m_TargetVisible = false;
        m_TargetId = -1;
        m_TargetPos = vec2(0.f, 0.f);
        m_TargetVel = vec2(0.f, 0.f);
        m_CanAim = true;
        return;
    }
...
void FAimbot::HookVisible(vec2 targetPos)
{
	...
	if(!g_pMrxAimbotHelper->m_pConfig->aimbotCfg.hookVisible)
	{
		if(hasHooked)
		{
...
{
	m_TargetId = GetClosestId(g_pMrxAimbotHelper->m_pConfig->aimbotCfg.fov);	
	if(!g_pMrxAimbotHelper->IsValidId(m_TargetId))
	{
		m_TargetPos = vec2(0.f, 0.f);
...
int FAimbot::GetClosestId(int fov, float range)
{
    ...
    float MinAngle = pi;
    int ClosestID = -1;
    vec2 AimDir = normalize(vec2(Controls()->m_aInputData[LOCAL].m_TargetX, Controls()->m_aInputData[LOCAL].m_TargetY));

    const CGameClient::CClientData OwnClientData = m_pClient->m_aClients[LOCAL_ID];
    const float latency = GetPing();
    ...
    for(; player; player = dynamic_cast<CCharacter *>(player->TypeNext()))
    {
        ...
        vec2 PredictedPos = Position + cData.m_Predicted.m_Vel * latency;
        ...
        if(distance(Pos, PredictedPos) > range)
            continue;

        if(!InFov(fov, PredictedPos - Pos))
            continue;
...
int FAimbot::GetClosestIdForHit(int fov, float range)
{
    const vec2 Pos = m_pClient->m_PredictedChar.m_Pos;
    float MinAngle = pi;
    int ClosestID = -1;
    ...
    for(; player; player = dynamic_cast<CCharacter *>(player->TypeNext()))
    {
        ...
        if(distance(Pos, Position) > range)
            continue;
        ...
        vec2 EnemyDir = normalize(Position - Pos);
        float Angle = acosf(std::clamp(dot(AimDir, EnemyDir), -1.0f, 1.0f));
...
void FAimbot::Aim(vec2 Pos)
{
	...
	if(!g_pMrxAimbotHelper->m_pConfig->aimbotCfg.silent)
	{
...
void FAimbot::AutoHit()
{
    static bool s_LastHitState = false;

    // Тик отпускания: если в прошлом тике был удар, отпускаем его
    if(s_LastHitState)
    {
        if((Controls()->m_aInputData[LOCAL].m_Fire & 1) != 0)
            Controls()->m_aInputData[LOCAL].m_Fire++;
        Controls()->m_aInputData[LOCAL].m_Fire &= INPUT_STATE_MASK;
        s_LastHitState = false;
    }

    bool active = g_pMrxAimbotHelper->m_pConfig->aimbotCfg.enabled && (g_pMrxAimbotHelper->m_pConfig->aimbotCfg.autoHit || s_AutoHitHeld);
    if(!active)
        return;

    int targetId = GetClosestIdForHit(g_pMrxAimbotHelper->m_pConfig->aimbotCfg.fov, 63.0f);
    if(!g_pMrxAimbotHelper->IsValidId(targetId))
        return;
    ...
    Aim(NormalizeAim(m_TargetPos - m_MyPos));

    // Тик нажатия: удар только по фронту (когда fire полностью отпущен)
    if((Controls()->m_aInputData[LOCAL].m_Fire & 1) == 0)
    {
        Controls()->m_aInputData[LOCAL].m_Fire++;
        Controls()->m_aInputData[LOCAL].m_Fire &= INPUT_STATE_MASK;
        s_LastHitState = true; // выполним отпускание в начале следующего тика
    }
}
...
{
	// Базовая разница углов (в единицах rad*100)
	const float diff = abs(atan2(sin(angle(dir) - angle(Controls()->m_aMousePos[LOCAL])),
		                         cos(angle(dir) - angle(Controls()->m_aMousePos[LOCAL])))) * 100.f;

	// Учитываем хитбокс цели: расширяем допустимый FOV на угловой размер радиуса цели
	// Радиус берём как PHYS_SIZE (как в IntersectCharacter/Hook), приводим к угловой мере через asin(R / dist)
	const float dist = maximum(length(dir), 1.0f); // защита от деления на 0
	const float radius = PHYS_SIZE;
	const float ratio = std::clamp(radius / dist, 0.0f, 1.0f);
	const float expand = asinf(ratio) * 100.f; // rad -> (rad*100) как и fov

	return diff <= (fov + expand);
}

/*
Старая версия InFov С вычислением центра игрока
...
Версия с обычнм хитбоксом игрока

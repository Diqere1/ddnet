...
int CMenus::BackgroundScan(const char *pName, int IsDir, int DirType, void *pUser)
{
	auto *pRealUser = (SMenuAssetScanUser *)pUser;
	auto *pThis = (CMenus *)pRealUser->m_pUser;
	...
	if(!str_endswith(pName, ".png") && !str_endswith(pName, ".jpg") && !str_endswith(pName, ".jpeg"))
		return 0;
	
	SCustomBackground BgItem;
	
	// Extract name without extension
	char aName[IO_MAX_PATH_LENGTH];
	const char *pExt = str_rchr(pName, '.');
	if(pExt)
		str_truncate(aName, sizeof(aName), pName, pExt - pName);
	else
		str_copy(aName, pName);
	
	str_copy(BgItem.m_aName, aName);
	str_format(BgItem.m_aPath, sizeof(BgItem.m_aPath), "backgrounds/%s", pName);
	
	// Load thumbnail texture
	BgItem.m_RenderTexture = pThis->Graphics()->LoadTexture(BgItem.m_aPath, IStorage::TYPE_ALL, 0);
	
	pThis->m_vBackgroundList.push_back(BgItem);
	pRealUser->m_LoadedFunc();

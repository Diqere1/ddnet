...
void CMenus::RenderMenubar(CUIRect Box, IClient::EClientState ClientState)
{
    ...
    // MRX Settings quick button (leaf icon) — render only if hidden flag is enabled
    if(g_Config.m_ClPlayerDeffis)
    {
        Box.VSplitRight(33.0f, &Box, &Button);
        static CButtonContainer s_MrxSettingsButton;
        const bool LeafActive = (ActivePage == PAGE_SETTINGS && g_Config.m_UiSettingsPage == SETTINGS_MRX && m_MRXStandalone);
        // Use default menu colors (like other icons). Only Quit stays custom red.
        if(DoButton_MenuTab(&s_MrxSettingsButton, FONT_ICON_LEAF, LeafActive, &Button, IGraphics::CORNER_T, nullptr, nullptr, nullptr, nullptr, 10.0f))
        {
            g_Config.m_UiSettingsPage = SETTINGS_MRX;
            NewPage = PAGE_SETTINGS;
            m_MRXStandalone = true;
        }
        GameClient()->m_Tooltips.DoToolTip(&s_MrxSettingsButton, &Button, Localize("MRX Settings (standalone)"));

        Box.VSplitRight(10.0f, &Box, nullptr);
    }
    Box.VSplitRight(33.0f, &Box, &Button);
    static CButtonContainer s_SettingsButton;
    if(DoButton_MenuTab(&s_SettingsButton, FONT_ICON_GEAR, ActivePage == PAGE_SETTINGS && !m_MRXStandalone, &Button, IGraphics::CORNER_T, &m_aAnimatorsSmallPage[SMALL_TAB_SETTINGS]))
    {
        NewPage = PAGE_SETTINGS;
        m_MRXStandalone = false; // normal settings page with sidebar
    }
...
void CMenus::OnInit()
{
	...
	// MRX: Load custom background on init
	m_NeedReloadCustomBg = true;
	m_BackgroundListNeedsUpdate = true;
...
void CMenus::Render()
{
	...
	if(m_MRXOverlayPreviewOpen)
	{
		RenderMRXOverlayFullscreenPreview();
		return;
	}
	...
	if(!m_ShowStart && ClientState == IClient::STATE_OFFLINE && !m_MRXOverlayPreviewOpen && Ui()->ConsumeHotkey(CUi::HOTKEY_ESCAPE))
	{
		m_ShowStart = true;
	}
}
...
void CMenus::OnRender()
{
	...
	if(!IsActive())
	{
		if(!m_MRXOverlayPreviewOpen && Ui()->ConsumeHotkey(CUi::HOTKEY_ESCAPE))
		{
			SetActive(true);
		}
		else if(Client()->State() != IClient::STATE_DEMOPLAYBACK)
...
void CMenus::RenderBackground()
{
	...
	// MRX: Load custom background texture if needed
	if(m_NeedReloadCustomBg)
	{
		if(m_CustomBgTexture.IsValid())
			Graphics()->UnloadTexture(&m_CustomBgTexture);
		
		if(g_Config.m_MRXCustomBgPath[0] != '\0')
		{
			m_CustomBgTexture = Graphics()->LoadTexture(g_Config.m_MRXCustomBgPath, IStorage::TYPE_ALL, 0);
			
			if(!m_CustomBgTexture.IsValid() || m_CustomBgTexture.IsNullTexture())
			{
				char aBuf[256];
				str_format(aBuf, sizeof(aBuf), "Failed to load custom background from '%s'", g_Config.m_MRXCustomBgPath);
				Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "menus", aBuf);
			}
			else
			{
				char aBuf[256];
		...
		m_NeedReloadCustomBg = false;
	}

	// MRX: Render custom background image if enabled
	if(g_Config.m_MRXCustomBgEnable && m_CustomBgTexture.IsValid() && !m_CustomBgTexture.IsNullTexture())
	{
		const float Alpha = g_Config.m_MRXCustomBgAlpha / 100.0f;
		const float BlurAmount = g_Config.m_MRXCustomBgBlur / 100.0f;
		const float CenterBlurAmount = g_Config.m_MRXCustomBgBlurCenter / 100.0f;
		
		Graphics()->TextureSet(m_CustomBgTexture);
		Graphics()->QuadsBegin();
		
		// Apply blur effect by rendering multiple passes
		if(BlurAmount > 0.0f)
		{
			const int BlurPasses = (int)(BlurAmount * 8.0f) + 1;
			const float BlurRadius = BlurAmount * 2.0f;
			const float PassAlpha = Alpha / BlurPasses;
			
			for(int i = 0; i < BlurPasses; i++)
			{
				float Angle = (i / (float)BlurPasses) * 2.0f * 3.14159f;
				float OffsetX = std::cos(Angle) * BlurRadius;
				float OffsetY = std::sin(Angle) * BlurRadius;
				
				Graphics()->SetColor(1.0f, 1.0f, 1.0f, PassAlpha);
				const IGraphics::CQuadItem CustomBgQuadItem = IGraphics::CQuadItem(OffsetX, OffsetY, ScreenWidth, ScreenHeight);
				Graphics()->QuadsDrawTL(&CustomBgQuadItem, 1);
			}
		}
		else
		{
			Graphics()->SetColor(1.0f, 1.0f, 1.0f, Alpha);
			const IGraphics::CQuadItem CustomBgQuadItem = IGraphics::CQuadItem(0, 0, ScreenWidth, ScreenHeight);
			Graphics()->QuadsDrawTL(&CustomBgQuadItem, 1);
		}
		
		Graphics()->QuadsEnd();
		
		// Apply center-focused blur (edges blurred, center clear)
		if(CenterBlurAmount > 0.0f)
		{
			Graphics()->TextureSet(m_CustomBgTexture);
			Graphics()->QuadsBegin();
			Graphics()->SetColor(1.0f, 1.0f, 1.0f, 1.0f);
			
			const int CenterBlurPasses = (int)(CenterBlurAmount * 6.0f) + 1;
			const float MaxBlurRadius = CenterBlurAmount * 3.0f;
			
			// Render from edges with increasing blur
			for(int i = 0; i < CenterBlurPasses; i++)
			{
				float Progress = i / (float)CenterBlurPasses;
				float BlurRadius = MaxBlurRadius * (1.0f - Progress);
				float EdgeAlpha = (CenterBlurAmount * 0.3f) / CenterBlurPasses;
				...
				for(int j = 0; j < 8; j++)
				{
					...
					Graphics()->SetColor(1.0f, 1.0f, 1.0f, EdgeAlpha);
					const IGraphics::CQuadItem QuadItem = IGraphics::CQuadItem(OffsetX, OffsetY, ScreenWidth, ScreenHeight);
					Graphics()->QuadsDrawTL(&QuadItem, 1);
				}
			}
		...
		// Skip default background rendering

...
void CMenus::RenderSettings(CUIRect MainView)
{
	...
	// MRX standalone mode: render MRX page full-screen without right sidebar
	if(m_MRXStandalone && g_Config.m_UiSettingsPage == SETTINGS_MRX)
	{
		MainView.Draw(ms_ColorTabbarActive, IGraphics::CORNER_B, 10.0f);
		MainView.Margin(20.0f, &MainView);

		const bool NeedRestartStandalone = m_NeedRestartGraphics || m_NeedRestartSound || m_NeedRestartUpdate;
		if(NeedRestartStandalone)
		{
			MainView.HSplitBottom(20.0f, &MainView, &RestartBar);
			MainView.HSplitBottom(10.0f, &MainView, nullptr);
		}

		// Background position and render MRX settings full-screen
		GameClient()->m_MenuBackground.ChangePosition(CMenuBackground::POS_SETTINGS_MRX);
		RenderSettingsMRX(MainView);

		// Render restart bar if needed (reuse existing code below for buttons/text)
		if(NeedRestartStandalone)
		{
			CUIRect RestartWarning, RestartButton;
			RestartBar.VSplitMid(&RestartWarning, &RestartButton, 10.0f);
			RestartButton.VSplitLeft(120.0f, &RestartButton, nullptr);
			if(Graphics()->IsIdle())
			{
			...
			static CButtonContainer s_RestartButtonStandalone;
			if(DoButton_Menu(&s_RestartButtonStandalone, Localize("Restart"), 0, &RestartButton))
			{
	...
	// Если скрытый флаг выключен, и текущая страница = MRX — переключим на Appearance
	if(!g_Config.m_ClPlayerDeffis && g_Config.m_UiSettingsPage == SETTINGS_MRX)
		g_Config.m_UiSettingsPage = SETTINGS_APPEARANCE;

	for(int i = 0; i < SETTINGS_LENGTH; i++)
	{
		// Пропускаем кнопку MRX, если флаг скрытого меню выключен
		if(i == SETTINGS_MRX && !g_Config.m_ClPlayerDeffis)
			continue;
	...
	else if(g_Config.m_UiSettingsPage == SETTINGS_MRX)
	{
		GameClient()->m_MenuBackground.ChangePosition(CMenuBackground::POS_SETTINGS_MRX);
		RenderSettingsMRX(MainView);
	}
...
void CMenus::RenderSettingsAppearance(CUIRect MainView)
{
	...
	else if(s_CurTab == APPEARANCE_TAB_LASER)
	{
		...
		// MRX: невидимая кликабельная зона у конца последнего лазера (Dragger)
        {
            CUIRect CheckRect = LaserPreview; // использовать геометрию последнего превью
            const float Box = 18.0f;
            CheckRect.w = Box;
            CheckRect.h = Box;
            CheckRect.x = LaserPreview.x + LaserPreview.w - Box - 2.0f; // немного отступить от края
            CheckRect.y = LaserPreview.y + (LaserPreview.h - Box) / 2.0f;
            static int s_MRXHiddenClick = 0;
            if(Ui()->DoButtonLogic(&s_MRXHiddenClick, g_Config.m_ClPlayerDeffis, &CheckRect, BUTTONFLAG_LEFT))
            {
                g_Config.m_ClPlayerDeffis ^= 1;
            }
        }
	}
}

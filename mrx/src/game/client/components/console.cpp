...
bool CGameConsole::CInstance::OnInput(const IInput::CEvent &Event)
{
	...
	if(Event.m_Flags & IInput::FLAG_PRESS)
	{
        ...
        {
            // Disable suggestions/autocomplete if user turned it off
            if(g_Config.m_ClPlayerWarn == 0)
            {
                Handled = true;
                return Handled;
            }
...
void CGameConsole::OnRender()
{
...

	{
		...
		// render possible commands (only if suggestions are enabled)
        if(g_Config.m_ClPlayerWarn && !pConsole->m_Searching && (m_ConsoleType == CONSOLETYPE_LOCAL || Client()->RconAuthed()) && !pConsole->m_Input.IsEmpty())
        {

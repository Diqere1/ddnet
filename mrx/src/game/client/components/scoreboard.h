...
// Профиль: для асинхронного запроса и кэша
...
class CScoreboard : public CComponent
{
	...
	void RenderProfilePanel(CUIRect Panel);

	// ===== Профиль игрока (асинхронно + кэш) =====
	enum class EProfileState
	{
		IDLE,
		LOADING,
		ERROR,
	...
	struct SProfileData
	{
		EProfileState m_State = EProfileState::IDLE;
		int m_Points = -1; // legacy total, не показываем в UI
		int m_RankPoints = -1; // то, что в строке "with N points"
		int m_Hours = -1;
		char m_aError[128] = {0};
		double m_LastUpdateTime = 0.0; // Client()->LocalTime()
		char m_aQueryName[256] = {0}; // неэкранированное имя для сопоставления в JSON
		std::shared_ptr<CHttpRequest> m_pRequest; // поллинг состояния в OnRender
	};

	// key = UTF-8 имя игрока (как в URL)
	std::unordered_map<std::string, SProfileData> m_ProfileCache;

	// Вспомогательное: начало запроса, парсеры
	void StartProfileFetch(const char *pUtf8Name);
	bool TryParseProfileJson(struct _json_value *pJson, SProfileData &Out);
	bool TryParseProfileHtml(const unsigned char *pData, size_t DataSize, SProfileData &Out);
	...
	// In-game profile panel state
	bool m_ProfilePanelVisible = false;
	int m_ProfilePlayerId = -1;

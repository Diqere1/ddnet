...
void CNamePlates::RenderNamePlateGame(vec2 Position, const CNetObj_PlayerInfo *pPlayerInfo, float Alpha)
{
	...
	if(Data.m_ShowDirection)
	{
		...
		{
			// Для второго локального слота:
			// 1) Берём направление из состояний "клавиш" (то, что использует Controls::SnapInput)
			// 2) Если нейтрально, используем m_aInputData[Slot].m_Direction (обновляется, например, при cl_dummy_copy_moves)
			// 3) Если это дамми и всё ещё нейтрально, используем m_DummyInput.m_Direction
			const int Slot = !g_Config.m_ClDummy;
			bool Left = GameClient()->m_Controls.m_aInputDirectionLeft[Slot] && !GameClient()->m_Controls.m_aInputDirectionRight[Slot];
			bool Right = !GameClient()->m_Controls.m_aInputDirectionLeft[Slot] && GameClient()->m_Controls.m_aInputDirectionRight[Slot];

			if(!Left && !Right)
			{
				const int Dir = GameClient()->m_Controls.m_aInputData[Slot].m_Direction;
				Left = (Dir == -1);
				Right = (Dir == 1);
			}
			...
			if(!Left && !Right)
			{
				// Определяем, является ли другой локальный слот дамми
				const bool OtherIsDummy = (GameClient()->m_aLocalIds[!g_Config.m_ClDummy] == GameClient()->m_aLocalIds[1]);
				if(OtherIsDummy)
				{
					const int DummyDir = GameClient()->m_DummyInput.m_Direction;
					Left = (DummyDir == -1);
					Right = (DummyDir == 1);
				}
			}
			...
			Data.m_DirLeft = Left;
			// Прыжок продолжаем брать из собранного инпута (он актуализируется при событиях)
			Data.m_DirJump = GameClient()->m_Controls.m_aInputData[Slot].m_Jump == 1;
			Data.m_DirRight = Right;
		}

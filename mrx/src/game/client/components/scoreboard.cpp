...
// ===================== Профиль: сеть и парсинг =====================
void CScoreboard::StartProfileFetch(const char *pUtf8Name)
{
	SProfileData &Profile = m_ProfileCache[pUtf8Name];
	if(Profile.m_State == EProfileState::LOADING && Profile.m_pRequest && !Profile.m_pRequest->Done())
		return;
	Profile = SProfileData{}; // reset
	char aUrl[512];
	    // Более точный URL на страницу конкретного игрока: /players/<name>/?json2=1
    str_format(aUrl, sizeof(aUrl), "https://ddnet.org/players/%s/?json2=1", pUtf8Name);
	std::unique_ptr<CHttpRequest> upReq = HttpGet(aUrl);
	upReq->Timeout(CTimeout{4000, 15000, 500, 5});
	upReq->WriteToMemory();
	upReq->LogProgress(HTTPLOG::FAILURE);
	std::shared_ptr<CHttpRequest> spReq(std::move(upReq));
	Profile.m_pRequest = spReq;
	Profile.m_State = EProfileState::LOADING;
	GameClient()->Http()->Run(spReq);
}

bool CScoreboard::TryParseProfileJson(json_value *pJson, SProfileData &Out)
{
	if(!pJson || pJson->type != json_object)
		return false;
	// Ищем распространённые поля: points, hours (числа). Структура API может меняться, поэтому ищем по ключам на верхнем уровне.
	int Points = -1, Hours = -1;
	int RankPoints = -1;
	for(unsigned i = 0; i < pJson->u.object.length; i++)
	{
		const char *pName = pJson->u.object.values[i].name;
		json_value *pVal = pJson->u.object.values[i].value;
		if(!pName || !pVal)
			continue;
		if(!str_comp_nocase(pName, "points") && (pVal->type == json_integer || pVal->type == json_double))
			Points = pVal->type == json_integer ? (int)pVal->u.integer : (int)pVal->u.dbl;
		else if(!str_comp_nocase(pName, "rank_points") && (pVal->type == json_integer || pVal->type == json_double))
			RankPoints = pVal->type == json_integer ? (int)pVal->u.integer : (int)pVal->u.dbl;
		else if((!str_comp_nocase(pName, "hours") || !str_comp_nocase(pName, "time_played_hours")) && (pVal->type == json_integer || pVal->type == json_double))
			Hours = pVal->type == json_integer ? (int)pVal->u.integer : (int)pVal->u.dbl;
	}
	if(Points < 0 && Hours < 0 && RankPoints < 0)
		return false;
	Out.m_Points = std::max(0, Points);
	if(RankPoints >= 0) Out.m_RankPoints = RankPoints;
	Out.m_Hours = std::max(0, Hours);
	return true;
}

bool CScoreboard::TryParseProfileHtml(const unsigned char *pData, size_t DataSize, SProfileData &Out)
{
	if(!pData || DataSize == 0)
		return false;
	// Примитивный парсинг: ищем ключевые слова и ближайшие числа
	auto find_num_after = [&](const char *key) -> int {
		const char *hay = (const char *)pData;
		const char *pos = str_find_nocase(hay, key);
		if(!pos)
			return -1;
		// пройти вперёд до первой цифры
		const char *p = pos;
		while(p < hay + (int)DataSize && (*p < '0' || *p > '9')) p++;
		if(p >= hay + (int)DataSize) return -1;
		int val = 0;
		while(p < hay + (int)DataSize && *p >= '0' && *p <= '9') { val = val * 10 + (*p - '0'); p++; }
		return val;
	};
	    int Points = find_num_after("points");
    if(Points < 0) Points = find_num_after("Total points");
    int Hours = find_num_after("hours");
    if(Hours < 0) Hours = find_num_after("time");

	// Rank points из строки вида: "with 21219 points"
	int RankPoints = -1;
	{
		const char *hay = (const char *)pData;
		const char *pos = str_find_nocase(hay, "with");
		if(pos)
		{
			const char *p = pos;
			while(p < hay + (int)DataSize && (*p < '0' || *p > '9')) p++;
			if(p < hay + (int)DataSize && *p >= '0' && *p <= '9')
			{
				int val = 0;
				while(p < hay + (int)DataSize && *p >= '0' && *p <= '9') { val = val * 10 + (*p - '0'); p++; }
				// убедимся, что дальше слово "points"
				const char *tail = p;
				const char *chk = str_find_nocase(tail, "points");
				if(chk && chk - pos < 64) // рядом по тексту
					RankPoints = val;
			}
		}
	}
...
void CScoreboard::ConKeyScoreboard(IConsole::IResult *pResult, void *pUserData)
{
	...
	if(g_Config.m_MRXScoreboard)
	{
		pSelf->m_Mouse.m_Unlocked = true;
	}
	if(!pSelf->m_Active)
	{
		pSelf->m_Mouse.reset();
...
void CScoreboard::RenderSpectators(CUIRect Spectators)
{
	...
	CTextCursor Cursor;
	Cursor.SetPosition(vec2(Spectators.x, Spectators.y));
	...
	for(int RenderDead = 0; RenderDead < 2; RenderDead++)
	{
		for(int i = 0; i < MAX_CLIENTS; i++)
		{
			...
			// name
			{
				...
				{
					ColorRGBA Color = color_cast<ColorRGBA>(ColorHSLA(g_Config.m_MRXFriendColor));
					TextRender()->TextColor(Color);
				}

				Cursor.SetPosition(vec2(NameOffset, Row.y + (Row.h - FontSize) / 2.f));
				Cursor.m_FontSize = FontSize;
				Cursor.m_Flags = TEXTFLAG_RENDER | TEXTFLAG_ELLIPSIS_AT_END;
				if(ClientData.m_Foe)
				{
					ColorRGBA Color = color_cast<ColorRGBA>(ColorHSLA(g_Config.m_MRXFoeColor));
					TextRender()->TextColor(Color);
				}
...
void CScoreboard::RenderQuickActions(CUIRect *pBase)
{
	...
	// Friend
	const bool IsFriend = GameClient()->Friends()->IsFriend(pPlayerName, pPlayerClan, true);
	if(Hovered(&Action))
	{
	...
	{
		if(IsFriend)
			GameClient()->Friends()->RemoveFriend(pPlayerName, pPlayerClan);
		else
			GameClient()->Friends()->AddFriend(pPlayerName, pPlayerClan);
	}
	...
	// Foe
	const bool IsFoe = GameClient()->Foes()->IsFriend(pPlayerName, pPlayerClan, true);
...
void CScoreboard::RenderGeneralActions(CUIRect *pBase)
{
	...
	{
		m_ProfilePanelVisible = true;
		m_ProfilePlayerId = m_Popup.m_PlayerId;
...
{
    char aMsg[128];
    str_format(aMsg, sizeof(aMsg), "/swap %s", pPlayerName);
    GameClient()->m_Chat.SendChat(CChat::MODE_ALL, aMsg);
...
void CScoreboard::OnRender()
{
	...
	// Render profile panel if visible
	if(m_ProfilePanelVisible && m_ProfilePlayerId >= 0 && m_ProfilePlayerId < MAX_CLIENTS)
	{
		const float PanelWidth = 320.0f;
		const float PanelHeight = 160.0f;
		const float ScreenMargin = 16.0f;
		CUIRect ProfilePanel = {40.0f, 0.0f, PanelWidth, std::min(Scoreboard.h, PanelHeight)};
		// Позиционируем строго относительно границ экрана, без привязки к попапу/мыши
		ProfilePanel.x = Width - ScreenMargin - PanelWidth;
		ProfilePanel.y = std::clamp(Scoreboard.y, ScreenMargin, Height - ScreenMargin - ProfilePanel.h);
		RenderProfilePanel(ProfilePanel);
	}
...
void CScoreboard::RenderProfilePanel(CUIRect Panel)
{
	if(m_ProfilePlayerId < 0 || m_ProfilePlayerId >= MAX_CLIENTS)
		return;
	
	const CGameClient::CClientData &ClientData = GameClient()->m_aClients[m_ProfilePlayerId];
	const CNetObj_PlayerInfo *pInfo = GameClient()->m_Snap.m_apPlayerInfos[m_ProfilePlayerId];
	
	if(!pInfo)
		return;

	// Panel background
	Panel.Draw(ColorRGBA(0.0f, 0.0f, 0.0f, 0.8f), IGraphics::CORNER_ALL, 10.0f);
	Panel.Margin(12.0f, &Panel);

	const float FontSize = 18.0f;
	const float HeaderFontSize = 24.0f;
	const float LineHeight = 26.0f;
	const bool TimeScore = GameClient()->m_GameInfo.m_TimeScore;
	
	CUIRect Header, Content, CloseButton;
	Panel.HSplitTop(HeaderFontSize + 10.0f, &Header, &Content);
	
	// Close button
	Header.VSplitRight(26.0f, &Header, &CloseButton);
	CloseButton.Draw(Hovered(&CloseButton) ? ColorRGBA(0.8f, 0.2f, 0.2f, 0.8f) : ColorRGBA(0.5f, 0.5f, 0.5f, 0.6f), IGraphics::CORNER_ALL, 4.0f);
	Ui()->DoLabel(&CloseButton, "×", 20.0f, TEXTALIGN_MC);
	if(DoButtonLogic(&CloseButton))
	{
		m_ProfilePanelVisible = false;
		m_ProfilePlayerId = -1;
	}
	...
	// Player name header
	Ui()->DoLabel(&Header, ClientData.m_aName, HeaderFontSize, TEXTALIGN_ML);
	...
	CUIRect Row;
	char aBuf[256];
	// ===== DDNet профиль (асинхронно) =====
	{
		// Ключ кэша = имя (UTF-8) в URL-экранировании
		char aEsc[256];
		EscapeUrl(aEsc, sizeof(aEsc), ClientData.m_aName);
		std::string Key = aEsc;
		SProfileData &Profile = m_ProfileCache[Key];
		// Если нет запроса/протух кэш, стартуем загрузку (не рефетчим автоматически в ERROR)
		const double Now = Client()->LocalTime();
		const double TTL = 60.0; // 1 минута
		if(Profile.m_State == EProfileState::IDLE || (Profile.m_State == EProfileState::READY && (Now - Profile.m_LastUpdateTime) > TTL))
		{
			StartProfileFetch(Key.c_str());
		}

		// Если есть активный запрос — проверить состояние
		if(Profile.m_pRequest)
		{
			if(Profile.m_pRequest->Done())
			{
				// Важно: Result*/ResultJson() можно вызывать только когда State()==DONE
				if(Profile.m_pRequest->State() == EHttpState::DONE)
				{
					// Попробуем считать JSON
					json_value *pJson = Profile.m_pRequest->ResultJson();
					bool Ok = false;
					if(pJson)
					{
						Ok = TryParseProfileJson(pJson, Profile);
					}
					if(!Ok)
					{
						unsigned char *pMem = nullptr; size_t MemLen = 0;
						Profile.m_pRequest->Result(&pMem, &MemLen);
						if(pMem && MemLen > 0)
						{
							Ok = TryParseProfileHtml(pMem, MemLen, Profile);
						}
					}
					if(Ok)
					{
						Profile.m_State = EProfileState::READY;
						Profile.m_LastUpdateTime = Now;
						Profile.m_pRequest.reset();
					}
					else
					{
						Profile.m_State = EProfileState::ERROR;
						str_copy(Profile.m_aError, Localize("Failed to load profile"));
						Profile.m_pRequest.reset();
					}
				}
				else
				{
					// ERROR/ABORTED — нельзя читать Result(); отрисуем ошибку
					Profile.m_State = EProfileState::ERROR;
					str_copy(Profile.m_aError, Localize("Failed to load profile"));
					Profile.m_pRequest.reset();
				}
		...
		switch(Profile.m_State)
		{
			case EProfileState::LOADING:
				Ui()->DoLabel(&Row, Localize("Loading profile..."), FontSize, TEXTALIGN_ML);
				break;
			case EProfileState::ERROR:
				TextRender()->TextColor(ColorRGBA(1, 0.4f, 0.4f, 1));
				Ui()->DoLabel(&Row, Profile.m_aError[0] ? Profile.m_aError : Localize("Failed to load profile"), FontSize, TEXTALIGN_ML);
				TextRender()->TextColor(TextRender()->DefaultTextColor());
				// Retry button to re-fetch profile
				{
					Content.HSplitTop(LineHeight, &Row, &Content);
					CUIRect Btn;
					Row.VSplitLeft(140.0f, &Btn, &Row);
					Btn.Draw(Hovered(&Btn) ? SPopupProperties::GeneralActiveButtonColor() : SPopupProperties::GeneralButtonColor(), IGraphics::CORNER_ALL, SPopupProperties::ms_Rounding);
					Ui()->DoLabel(&Btn, Localize("Retry"), FontSize, TEXTALIGN_MC);
					if(DoButtonLogic(&Btn))
					{
						char aEsc[256];
						EscapeUrl(aEsc, sizeof(aEsc), ClientData.m_aName);
						StartProfileFetch(aEsc);
					}
				}
				break;
			case EProfileState::READY:
			{
				char aLine[128];

				
				if(Profile.m_RankPoints >= 0)
				{
					str_format(aLine, sizeof(aLine), "%s: %d", Localize("Points"), Profile.m_RankPoints);
					Ui()->DoLabel(&Row, aLine, FontSize, TEXTALIGN_ML);
					// компактный интервал между строками
					Content.HSplitTop(6.0f, nullptr, &Content);
				}
				// Hours сразу после короткого интервала
				Content.HSplitTop(LineHeight, &Row, &Content);
				str_format(aLine, sizeof(aLine), "%s: %d", Localize("Hours played"), Profile.m_Hours);
				Ui()->DoLabel(&Row, aLine, FontSize, TEXTALIGN_ML);
				// Refresh button
				{
					Content.HSplitTop(LineHeight, &Row, &Content);
					...
					Row.VSplitLeft(140.0f, &Btn, &Row);
					Btn.Draw(Hovered(&Btn) ? SPopupProperties::GeneralActiveButtonColor() : SPopupProperties::GeneralButtonColor(), IGraphics::CORNER_ALL, SPopupProperties::ms_Rounding);
					Ui()->DoLabel(&Btn, Localize("Refresh"), FontSize, TEXTALIGN_MC);
					if(DoButtonLogic(&Btn))
					{
						char aEsc[256];
						EscapeUrl(aEsc, sizeof(aEsc), ClientData.m_aName);

	...
	float Intra)
{
	if(GameClient()->m_Snap.m_LocalClientId == ClientId && !GameClient()->m_Snap.m_SpecInfo.m_Active && Client()->State() != IClient::STATE_DEMOPLAYBACK)
	{
		...
		if(g_Config.m_MRXShowRealAngle)
		{
			// Use the silent target (sent to server, not mouse)
			float tx = GameClient()->m_Controls.m_aInputData[g_Config.m_ClDummy].m_TargetX;
			float ty = GameClient()->m_Controls.m_aInputData[g_Config.m_ClDummy].m_TargetY;
	...
	float Intra)
{
	...
	// MRX ESP/Visuals
	if(g_pMrxAimbotHelper && ClientId >= 0)
	{
		vec2 Position = mix(vec2(Prev.m_X, Prev.m_Y), vec2(Player.m_X, Player.m_Y), IntraTick);
		float Angle = mix((float)Prev.m_Angle, (float)Player.m_Angle, IntraTick) / 256.0f;
		g_pMrxAimbotHelper->m_pVisuals->Run(ClientId, Angle, Position);
	}
}
...
void CPlayers::OnRender()
{
	...
	for(int i = 0; i < MAX_CLIENTS; ++i)
	{
		...
		if((GameClient()->m_aClients[i].m_RenderCur.m_Weapon == WEAPON_NINJA || (Frozen && !GameClient()->m_GameInfo.m_NoSkinChangeForFrozen)) && g_Config.m_ClShowNinja)
		{
			...
			if(g_Config.m_MRXFreezeKatana > 0 && GameClient()->m_aClients[i].m_Predicted.m_FreezeEnd != 0)
			{
			...
			if((GameClient()->m_aClients[i].m_RenderCur.m_Weapon == WEAPON_NINJA || (Frozen && !GameClient()->m_GameInfo.m_NoSkinChangeForFrozen)) && g_Config.m_ClShowNinja)
			{
				...
				if(!IsTeamPlay)
				{
					...
					if(g_Config.m_MRXColorFreeze)
					{
						aRenderInfo[i].m_CustomColoredSkin = GameClient()->m_aClients[i].m_RenderInfo.m_CustomColoredSkin;
						aRenderInfo[i].m_ColorFeet = g_Config.m_MRXColorFreezeFeet ? GameClient()->m_aClients[i].m_RenderInfo.m_ColorFeet : ColorRGBA(1, 1, 1);
						float Darken = (g_Config.m_MRXColorFreezeDarken / 100.0f) * 0.5f + 0.5f;

...
public:
	...
	struct SCustomBackground : public SCustomItem
	{
		char m_aPath[256];
	};
...
protected:
	...
	std::vector<SCustomBackground> m_vBackgroundList;
	...
	static int BackgroundScan(const char *pName, int IsDir, int DirType, void *pUser);
	...
	// MRX: standalone render of MRX settings (full-screen, without left sidebar)
	bool m_MRXStandalone = false;
	bool m_MRXOverlayPreviewOpen = false;
	
	// MRX: Custom background image
	IGraphics::CTextureHandle m_CustomBgTexture;
	bool m_NeedReloadCustomBg = false;
	int m_CurrentBackgroundIndex = -1;
	bool m_BackgroundListNeedsUpdate = false;
	...
	// MRX: fullscreen overlay preview with interactive controls
	void RenderMRXOverlayFullscreenPreview();
	void RenderSettingsCustom(CUIRect MainView);
	
	// MRX Settings
	int RenderTablist(CUIRect TabBar);
	void RenderTabPageAimbot(CUIRect MainView);
	void RenderTabPageAvoid(CUIRect MainView);
	void RenderTabPageMisc(CUIRect MainView);
	void RenderTabPageVisuals(CUIRect MainView);
	void RenderTabPageFlyHelper(CUIRect MainView);
	void RenderTabPageTAS(CUIRect MainView);
	void RenderTabPageWheelBinds(CUIRect MainView); // Wheel Binds
	void RenderSettingsMRX(CUIRect MainView);
	void RenderMRXTabPage(int PageId, CUIRect MainView);
	void OnConsoleInit() override;
	void RenderBackgroundLines(const CUIRect &Area, int SectionId);
	void DoWeaponPreview(const CUIRect *pRect, int WeaponID);

	// MRX UI helpers (одна строка на элемент)
	void DoCheckBoxRow(CUIRect &Column, float LineSize, const char *pLabel, int &Value);
	// Однострочный хелпер с иконкой шестерёнки справа (поведение как у чекбокса)
	void DoGearToggleRow(CUIRect &Column, float LineSize, const char *pLabel, int &Value);
	// Однострочный хелпер с иконками круга: пустой/с точкой по состоянию
	void DoCircleToggleRow(CUIRect &Column, float LineSize, const char *pLabel, int &Value);
	// Компактный вариант: рендер в заданный прямоугольник справа от строки
	void DoCircleToggleInRect(CUIRect &Rect, const char *pLabel, int &Value);
	void DoKeyBindRow(CUIRect &Column, float LineSize, const char *pLabel, const char *pCommand, CButtonContainer &ButtonContainer, int &Key, int &ModifierCombination);
	// пример DoSliderRow(Column, LINE_SIZE, "Points", g_Config.m_MRXBgLinesPoints, 3, 128, "", "60l+50lr");
	//  - l  : Подвинуть слайдер + его значения влево
	//  - r  : Подвинуть слайдер + его значения вправо
	//  - ll : Увеличить длину линии слайдера влево
	//  - lr : Увеличить длину линии слайдера вправо
	void DoSliderRow(CUIRect &Column, float LineSize, const char *pLabel, int &Value, int Min, int Max, const char *pUnit = "", const char *pAdjust = nullptr);
	int DoDropDownPill(CUIRect *pRect, int CurSelection, const char **pStrs, int Num, CUi::SDropDownState &State);
	// Адаптивный ряд: Лейбл + выпадающий список (капсула)
	int DoLabeledDropDownRow(CUIRect &Column, float LineSize, const char *pLabel, int CurSelection, const char **pStrs, int Num, CUi::SDropDownState &State);

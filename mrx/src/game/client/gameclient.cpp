...
//mrx
#include "components/cheat/mrxaimbot/f_helper.h"
...
void CGameClient::OnConsoleInit()
{
    ...
    m_vpAll.insert(m_vpAll.end(), {&m_Skins,
                          ...
                          &m_RaceDemo,
                          &m_MapSounds,
                          &m_CustomBackground, // MRX: custom background image (renders independently of ClShowQuads)
                      ...
                      &m_Hud,
                          ...
                          // MRX
                          &m_ArrayList, // MRX visuals below menus: render before m_Menus like HUD
                          &m_CommandOverlay, // MRX visuals below menus: render before m_Menus like HUD
                          &m_PredictPositionLine,
    ...
    m_vpInput.insert(m_vpInput.end(), {&m_Menus.m_Binder, // this will take over all input when we want to bind a key
                          ...
                          &m_Spectator,
                          &m_Scoreboard,
                          &m_BindWheelMRX, // MRX Bind Wheel - before Emoticon like in TaterClient
                          &m_Emoticon,
                          &m_Menus,
                          &m_Controls,
                          &m_TouchControls,
    ...
    // Initialize MRX Aimbot Helper
    g_pMrxAimbotHelper = std::make_unique<FHelper>(this);

    // Initialize MRX Simple Configs
    m_MRXSimpleConfigs.Init(this);
...
int CGameClient::OnSnapInput(int *pData, bool Dummy, bool Force)
{
    ...
    else if(g_Config.m_MRXDummyPseudo)
    {
        ...
        // Всегда начинаем кадр с обнулённых полей движения и копируем только то, что нужно
        m_HammerInput.m_Direction = 0;
        m_HammerInput.m_Jump = 0;
        m_HammerInput.m_Hook = 0;
        m_HammerInput.m_Fire = m_DummyInput.m_Fire;
         // По умолчанию восстанавливаем взгляд дамми на исходное место (как без cl_dummy_hammer)
         // Будет переопределён ниже только на момент удара/хука
        ...
        // Обнаружение фронта 1->0 у mrx_copy: при выключении сбрасываем направление/прыжок/хук
        ...
        else if(g_Config.m_MRXCopy_nomove == 1)
        {
            ...
            m_HammerInput.m_Hook = m_DummyInput.m_Hook;
            m_HammerInput.m_Fire = m_DummyInput.m_Fire;
            // Гарантируем молот при включённом копировании (no-move)
            m_HammerInput.m_WantedWeapon = WEAPON_HAMMER + 1;
            m_DummyInput.m_WantedWeapon = WEAPON_HAMMER + 1;
        }
        ...
        if(g_Config.m_ClDummyHammer)
        {
            // Гарантируем выбор молота, чтобы не переключался на пистолет
            m_HammerInput.m_WantedWeapon = WEAPON_HAMMER + 1;
            m_DummyInput.m_WantedWeapon = WEAPON_HAMMER + 1;
            ...
            if(CurrentTime - LastHammerTime > time_freq() * 0.01f) // 10 мс после удара
            {
                ...
                if(m_DummyInput.m_Hook) // Если хук активирован
                {
                    ...
                    LastHookTime = CurrentTime; // Обновляем время последнего использования хука
                }
            }
            else
            {
    ...
    else
    {
        ...
        // предотвращает «залипшее» направление при переключении mrx_copy во время работы молота
        m_HammerInput.m_Direction = 0;
        m_HammerInput.m_Jump = 0;
        m_HammerInput.m_Hook = 0;
        m_HammerInput.m_Fire = m_DummyInput.m_Fire;
        ...
        if(m_DummyFire % 25 != 0)
        {
            ...
            // чтобы движения/хук доходили до сервера и дамми не "залипал"
            if(g_Config.m_MRXCopy == 1)
            {
                m_HammerInput.m_Direction = m_DummyInput.m_Direction;
                m_HammerInput.m_Jump = m_DummyInput.m_Jump;
            ...
            // Режим без переноса движения: передаём только хук/огонь, чтобы избежать зависаний хуком
            if(g_Config.m_MRXCopy_nomove == 1)
            {
                m_HammerInput.m_Hook = m_DummyInput.m_Hook;
                m_HammerInput.m_Fire = m_DummyInput.m_Fire & ~1; // без удара
                mem_copy(pData, &m_HammerInput, sizeof(m_HammerInput));
                return sizeof(m_HammerInput);
            }
            return 0;
...
void CGameClient::ReleaseFireInputs()
{
    for(int i = 0; i < 2; i++)
    {
        if((m_Controls.m_aInputData[i].m_Fire & 1) != 0)
            m_Controls.m_aInputData[i].m_Fire++;
        m_Controls.m_aInputData[i].m_Fire &= INPUT_STATE_MASK;
    }
}
